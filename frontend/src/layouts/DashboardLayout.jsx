...
export default function DashboardLayout({ children }) {
  ...
  const [desktopSidebarCollapsed, setDesktopSidebarCollapsed] = useState(() => localStorage.getItem('desktopSidebarCollapsed') === 'true');

  useEffect(() => {
    localStorage.setItem('desktopSidebarCollapsed', desktopSidebarCollapsed);
  ...
  const navItems = [
    ...
    { name: 'Failed Questions', path: '/failed-questions', icon: AlertCircle },
    { name: 'Spam Questions', path: '/spam-questions', icon: ShieldAlert },
        ...
        <div className={`flex items-center p-4 border-b border-gray-200 h-16 shrink-0 ${desktopSidebarCollapsed && !sidebarOpen ? 'justify-center' : 'justify-between'}`}>
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-8 h-8 shrink-0 rounded-xl bg-blue-600 flex items-center justify-center overflow-hidden">
              <img src={getLogoUrl(userData?.dashboard_logo_url, logo)} alt="Logo" className="w-full h-full object-cover" />
            </div>
            {(!desktopSidebarCollapsed || sidebarOpen) && (
              <div className="whitespace-nowrap transition-opacity duration-300">
                ...
                <span className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Subscriber Portal</span>
              </div>
            )}
          </div>
        ...
        <header className="h-16 shrink-0 flex items-center justify-between px-6 bg-white border-b border-gray-200 z-[50] sticky top-0">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg font-semibold text-gray-900 truncate">
              {navItems.find(i => (i.exact ? pathname === i.path : pathname.startsWith(i.path)))?.name || 'Dashboard'}
            </h2>
          </div>
